...
- **Vocabulaire métier de la commande (règle validée Lot 6)** : « Demande d'achat → Commande → Réceptions partielles → Clôture » se représente techniquement par la machine d'état `Order`/`OrderStatus` existante (inchangée depuis le Lot 5 : Brouillon ≈ Demande d'achat, Active ≈ Commande, Clôturée ≈ Clôture) complétée par des événements répétables `OrderReceipt` (Réceptions partielles, append-only, jamais modifiés — une correction est une nouvelle réception, éventuellement à montant/jours négatifs). Ce vocabulaire ne doit jamais être réintroduit comme un nouveau jeu de statuts : la machine d'état testée au Lot 5 prévaut.
...
- **Intégration continue (Lot 13)** : `.github/workflows/ci.yml` s'exécute sur chaque Pull Request et chaque push vers `main` (build + tests + couverture backend et frontend). Aucune branch protection bloquante n'est configurée à ce jour — la vérification manuelle des checks avant fusion reste de mise.
...
- `IRepository<T>.RemoveAsync` (Lot 6) : suppression physique **réservée aux relations de rattachement pur sans portée historique/métier** (ex. `UserPermission`, retrait d'une permission) — jamais utilisée pour une entité couverte par la règle « statut plutôt que suppression physique » (temps, absences, projets, imports, journaux, ci-dessus). L'entité passée peut être un « stub » ne portant que sa clé (composite ou non) : EF Core émet un `DELETE` par clé sans nécessiter un chargement préalable.
...
- **Pattern d'écriture `AuditService` (Lot 6)** : `AuditService.RecordAsync(...)` ajoute l'entrée au suivi EF Core (`IRepository<AuditLog>.AddAsync`) mais n'appelle **jamais** `SaveChangesAsync` lui-même. C'est l'appel de `SaveChangesAsync` déjà fait par le service métier appelant (sur le changement qu'il décrit) qui persiste les deux ensembles de modifications dans une seule transaction EF Core — « même transaction que le changement métier » sans introduire de nouvelle abstraction de type Unit of Work. Conséquence directe : `RecordAsync` doit toujours être appelé **avant** le `SaveChangesAsync` de l'opération métier concernée, jamais après ni dans un `Task` détaché.
- Codes d'action stables dans `AuditActions` (`Application/Audit/AuditActions.cs`), chaîne extensible plutôt qu'un enum fermé (la liste du §28.3 est un minimum, pas un catalogue clos) — même principe que `PermissionCodes`.
- Contexte technique (adresse IP) porté par l'abstraction `IAuditContextAccessor` (Application), implémentée par `HttpAuditContextAccessor` (Api) — même principe que `ICurrentUser` : `AuditService` ne dépend jamais de `HttpContext` directement.
...
- Authentification de démonstration acceptée pour le MVP, mais l'architecture doit permettre une intégration future avec Active Directory, LDAP, OpenID Connect ou un SSO d'entreprise, sans refonte du modèle d'autorisation. L'identité de l'appelant est portée par l'abstraction `ICurrentUser` (couche `Application`) ; son implémentation actuelle (`DemoCurrentUserProvider`, Api) lit une identité déjà résolue par `IdentityResolutionMiddleware` dans `HttpContext.Items`. Contrôleurs, services applicatifs et règles de sécurité ne dépendent que de `ICurrentUser` : remplacer l'implémentation par un futur provider LDAP/OIDC ne les modifie pas.
- **Session simulée sessionnée (Lot 13)** : `IAuthenticationProvider` (Application, `Common/Security/`) est la seule abstraction du cycle de vie de session (création/révocation/résolution), implémentée par `DemoAuthenticationProvider` (Api) — toujours pas de mot de passe, de compte local ni de JWT définitif, mais une session serveur réelle (`UserSession`, jeton opaque en cookie HttpOnly/`SameSite=Strict`/`Secure` hors Development, expiration glissante). `UserSession.IsPersistent` distingue déjà une session navigateur d'une session persistante ("se souvenir de moi", non exposée à l'écran ce lot) pour que cette fonctionnalité future n'exige aucune migration supplémentaire. Façade de compatibilité pour les tests d'intégration existants : `Authentication:AllowDirectDemoHeader` (`true` en Development/Test uniquement) laisse l'en-tête `X-Demo-User` résoudre une identité sans session, jamais en Qualification/Production.
...
- **Une permission dédiée par action sensible plutôt qu'une réutilisation d'axe existant** (précédent `TIME_ENTRY_CORRECTION`, Lot 5, étendu au Lot 6) : `TIME_ENTRY_RECALCULATION` (recalcul explicite, §19.6), `USER_ADMINISTRATION` (modification/désactivation d'utilisateur, changement de rôle, octroi/retrait de permission, §28.3), `IMPORT_EXECUTE` (exécution d'un import, §27 — un import portant sur un type financier exige en plus `FINANCIAL_DATA_VIEW`), `AUDIT_VIEW` (consultation du journal d'audit, §28.1), `ORDER_RECEIPT_OVERRIDE` (dépassement du reste réceptionnable d'une commande, règle métier validée Lot 6). Codes centralisés dans `PermissionCodes` (`Application/Common/Security/`).
- **Sécurisation API (Lot 13)** : en-têtes de sécurité systématiques (HSTS hors Development, `X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy`, `Content-Security-Policy: default-src 'none'` — l'API ne sert que du JSON) ; limitation de débit native ASP.NET Core (`Microsoft.AspNetCore.RateLimiting`) sur la création de session ; limite de taille de requête sur les endpoints d'upload d'import (§27).
...
- Moteur générique unique (`Application/Imports/Services/ImportService.cs`) partagé par les 16 types importables du cahier des charges §27.1 : aperçu (`Preview`, aucune écriture) → simulation (`SimulateAsync`, aucune écriture) → exécution (`ExecuteAsync`, écrit `ImportBatch`/`ImportDiff` et audite). Chaque type fournit uniquement un **adaptateur** (`IImportAdapter`, `Application/Imports/Adapters/`) qui réutilise les services métier applicatifs déjà écrits (Create/Update/action dédiée) — le moteur d'import n'accède **jamais** directement au contexte EF Core ni à `IRepository<T>` d'écriture pour les entités importées (règle validée à l'ouverture du Lot 6). Seuls `ImportBatch`/`ImportDiff` (propres au moteur lui-même) sont écrits directement par `ImportService`.
- `SupportedModes` par adaptateur reflète strictement ce que le service métier sous-jacent expose déjà : Mise à jour n'est proposée que si une méthode `UpdateAsync` existe ; Complet uniquement si l'entité porte un mécanisme d'archivage/désactivation déjà exposé (`ArchiveAsync`/`DeactivateAsync`/`DeleteAsync`/`CloseAsync`) **et** que le type est un référentiel global non scopé à un parent (Project, User, TimeEntry, Budget) — un sous-type scopé à un parent (`ProjectParticipant`, `Plannings`) n'active jamais Complet : le moteur n'a pas de notion de « périmètre de fichier » pour garantir qu'un fichier ne décrivant qu'un seul parent n'archive pas les entités des autres parents. Aucune méthode de service n'est créée dans ce lot uniquement pour satisfaire l'import.
- Mapping CSV → DTO : MVP sans assistant de correspondance interactif (§27.3 étape 5, différée au frontend futur — aucun écran n'existe pour aucun lot à ce jour). Les en-têtes CSV attendus correspondent exactement aux noms de propriété du DTO cible (`CsvRequestBinder.Bind<T>`, réflexion générique unique — même principe que Mapster, déjà accepté comme mapping par réflexion) ; exposés par `GET /api/v1/imports/types`. Une colonne `Id` renseignée = mise à jour de cette entité ; absente = ajout. Les colonnes de clé étrangère attendent l'identifiant technique (GUID), jamais une résolution par clé métier/nom.
- Diff de mise à jour calculé par `FieldDiffer.Diff` (réflexion générique, compare uniquement les propriétés portées par le DTO de requête) — jamais une comparaison écrite à la main par type.
- Import SharePoint simulé (§27.4) : même pipeline, `source = "SharePoint"` sur `ImportBatch`. Comparaison « au chargement précédent » = dernier `ImportBatch` confirmé du même type/de la même source (`PreviousBatchId`), diffée contre l'état actuel de la base (qui reflète déjà le dernier import) — pas de stockage d'un fichier précédent brut. Point d'extension pour un futur connecteur SharePoint réel : l'obtention des octets du fichier, jamais la logique de comparaison/exécution.
