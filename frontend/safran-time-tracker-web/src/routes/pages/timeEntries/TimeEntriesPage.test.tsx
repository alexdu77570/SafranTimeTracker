...
vi.mock('./TimeEntryForm', () => ({
  TimeEntryCreateForm: ({ seed, onSuccess }: { seed?: { reference?: string }; onSuccess: () => void }) => (
    <div>
...
vi.mock('../../../api/endpoints/timeEntries', () => ({ fetchTimeEntries, deleteTimeEntry, recalculateTimeEntry }))

vi.mock('../../../api/endpoints/activityTypes', () => ({
  fetchActivityTypes: vi.fn(async () => ({
    items: [
      { id: 'run', code: 'RUN', libelle: 'RUN', isRun: true, referenceRequired: false, referenceFormatRegex: null, referenceExample: null, statut: 0 },
      {
        id: 'incident', code: 'INCIDENT', libelle: 'Incident', isRun: true, referenceRequired: true,
        referenceFormatRegex: '^INC\\d{7}$', referenceExample: 'INC0001234', statut: 0,
      },
    ],
...
vi.mock('../../../api/endpoints/resources', () => ({
  fetchResources: vi.fn(async () => ({
    items: [{ id: 'resource-1', nom: 'BERNARD', prenom: 'Alexandre', departmentId: 'd', serviceId: 's', teamId: null, responsableHierarchiqueId: null, companyId: null, defaultOrderId: null, dailyCapacity: 7.75, weeklyCapacity: 38.75, statut: 0, commentaire: null, operationalRoleIds: [] }],
    page: 1, pageSize: 100, totalCount: 1,
  })),
}))
vi.mock('../../../api/endpoints/projects', () => ({
  fetchProjects: vi.fn(async () => ({
    items: [{ id: 'project-1', nom: 'Migration ELM', code: 'PRJ-ELM-2026', applicationId: 'a', piloteId: 'p', departmentId: 'd', serviceId: 's', teamId: null, statusId: 'st', projectTypeId: null, clientId: null, dateDebut: '2026-01-01', dateFinPrevueInitiale: '2026-12-31', dateFinAjustee: null, dateFinReelle: null, niveauRisque: 0, commentaire: null }],
    page: 1, pageSize: 100, totalCount: 1,
  })),
}))
vi.mock('../../../api/endpoints/orders', () => ({
  fetchOrders: vi.fn(async () => ({
    items: [{ id: 'order-1', reference: 'CMD-2026-0001', libelle: 'Commande démo', companyId: 'c', projectId: null, budgetFinancierInitial: 0, budgetFinancierAjuste: 0, budgetJoursInitial: null, budgetJoursAjuste: null, dateDebut: '2026-01-01', dateFinInitiale: '2026-12-31', dateFinAjustee: null, statusId: 'st', seuilAlerte: null, commentaire: null }],
    page: 1, pageSize: 100, totalCount: 1,
  })),
}))
vi.mock('../../../api/endpoints/users', () => ({
  fetchUsers: vi.fn(async () => ({
    items: [{
      id: 'user-1', nom: 'BERNARD', prenom: 'Alexandre', identifiant: 's636140', email: 's636140@safran.local', telephone: null,
      statut: 0, dateArrivee: '2021-01-01', dateSortie: null, commentaire: null, resourceId: 'resource-1', roleId: 'role-1',
      accesGlobal: true, permissionIds: ['perm-recalc'],
    }],
    page: 1, pageSize: 100, totalCount: 1,
  })),
}))
vi.mock('../../../api/endpoints/permissions', () => ({
  fetchPermissions: vi.fn(async () => ({
    items: [{ id: 'perm-recalc', code: 'TIME_ENTRY_RECALCULATION', libelle: 'Recalcul', description: null }],
    page: 1, pageSize: 100, totalCount: 1,
  })),
}))

const entry = {
  id: 'te-1', resourceId: 'resource-1', activityTypeId: 'run', projectId: null, orderId: null,
  date: '2026-07-15', dureeHeures: 7.75, reference: null, commentaire: null, statut: 0,
  createdAt: '2026-07-15T00:00:00Z', createdBy: 's636140', updatedAt: null, updatedBy: null,
  financialSnapshot: {
    tjmPersonneSnapshot: 500, sourceTjmPersonne: 'ResourceTjmHistory', resourceTjmHistoryId: 'h1',
    tjmContratSnapshot: null, sourceContrat: null, companyContractHistoryId: null, companyIdSnapshot: 'c1',
    coutReelCalcule: 500, coutContratCalcule: null, differentielCalcule: null,
    calculationDate: '2026-07-15T00:00:00Z', calculationStatus: 0,
  },
}
...
describe('TimeEntriesPage', () => {
  ...
  it('re-queries with the selected activity type, project and order filters', async () => {
    ...
    await waitFor(() => expect(fetchTimeEntries).toHaveBeenCalledWith(expect.objectContaining({ activityTypeId: 'incident' })))

    await user.click(screen.getByLabelText('Projet'))
    await user.click(await screen.findByRole('option', { name: 'Migration ELM' }))
    await waitFor(() => expect(fetchTimeEntries).toHaveBeenCalledWith(expect.objectContaining({ projectId: 'project-1' })))

    await user.click(screen.getByLabelText('Commande'))
    await user.click(await screen.findByRole('option', { name: 'CMD-2026-0001' }))
    await waitFor(() => expect(fetchTimeEntries).toHaveBeenCalledWith(expect.objectContaining({ orderId: 'order-1' })))
  })
  ...
  it('sets Du/Au to the ISO week bounds when a day is picked in the "Semaine" filter (§19.4)', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Semaine (jour contenu)'), { target: { value: '2026-07-15' } })

    const bounds = weekBounds('2026-07-15')
    await waitFor(() =>
      expect(fetchTimeEntries).toHaveBeenCalledWith(expect.objectContaining({ from: bounds.start, to: bounds.end })),
    )
  ...
  it('creates a duplicated entry from an existing row and refreshes the list', async () => {
    ...
    await waitFor(() => expect(fetchTimeEntries).toHaveBeenCalled())
    await waitFor(() => expect(screen.queryByText('Formulaire de création')).not.toBeInTheDocument())
  })
