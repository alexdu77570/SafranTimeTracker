...
describe('periodBounds', () => {
  it('resolves monthly bounds around the reference date', () => {
    expect(periodBounds('mensuelle', '2026-07-20')).toEqual({ start: '2026-07-01', end: '2026-07-31' })
  })

  it('resolves weekly bounds around the reference date (locale française, semaine du lundi)', () => {
    // 2026-07-20 est un lundi.
    expect(periodBounds('hebdomadaire', '2026-07-20')).toEqual({ start: '2026-07-20', end: '2026-07-26' })
  })
})
...
vi.mock('../../../api/endpoints/organisation', () => ({
  fetchDepartments: vi.fn(async () => ({
    items: [{ id: 'dept-1', code: 'DSI', nom: 'DSI', responsableId: null, statut: 0, commentaire: null }],
    page: 1, pageSize: 100, totalCount: 1,
  })),
  fetchServices: vi.fn(async () => ({
    items: [{ id: 'service-1', code: 'PROD', nom: 'Production applicative', departmentId: 'dept-1', responsableId: null, statut: 0, commentaire: null }],
    page: 1, pageSize: 100, totalCount: 1,
  })),
...
vi.mock('../../../api/endpoints/users', () => ({
  fetchUsers: vi.fn(async () => ({
    items: [{
      ...
      accesGlobal: true, permissionIds: [],
    }],
    page: 1, pageSize: 100, totalCount: 1,
  })),
}))
...
function resource(id: string, prenom: string, nom: string) {
  return { id, nom, prenom, departmentId: 'dept-1', serviceId: 'service-1', teamId: null, responsableHierarchiqueId: null, companyId: null, defaultOrderId: null, dailyCapacity: 7.75, weeklyCapacity: 38.75, statut: 0, commentaire: null, operationalRoleIds: [] }
}

function availability(resourceId: string, overrides: Partial<{ tauxDisponibilite: number }> = {}) {
  return {
    resourceId, startDate: '2026-07-01', endDate: '2026-07-31', joursOuvres: 23, joursFeries: 0, joursAbsenceValidee: 0,
    capaciteTheorique: 178.25, capaciteReelle: 178.25, tauxDisponibilite: 100, chargeRunHeures: 0, chargeHorsRunHeures: 0,
    ...overrides,
  }
}
...
describe('AvailabilityPage', () => {
  it('displays the capacity-per-resource table and the colored calendar for the current identity', async () => {
    ...
    fetchResources.mockResolvedValue({ items: [resource('resource-1', 'Alexandre', 'BERNARD')], page: 1, pageSize: 100, totalCount: 1 })
    ...
    await waitFor(() => expect(screen.getAllByText('Disponible', { selector: 'span' }).length).toBeGreaterThan(0))
  })
  ...
  it('re-queries resources when the département filter changes', async () => {
    ...
    await waitFor(() => expect(fetchResources).toHaveBeenCalledWith(expect.objectContaining({ departmentId: 'dept-1' })))
  })
  ...
  it('refetches with weekly bounds when switching to the Hebdomadaire view', async () => {
    ...
    renderPage()
    await screen.findAllByText('Alexandre BERNARD')
    fireEvent.change(screen.getByLabelText('Période de référence'), { target: { value: '2026-07-20' } })
  ...
  it('switches the colored calendar to the resource clicked in the table', async () => {
    ...
    fetchResources.mockResolvedValue({
      items: [resource('resource-1', 'Alexandre', 'BERNARD'), resource('resource-2', 'Camille', 'DURAND')],
    ...
    await waitFor(() => expect(fetchAbsences).toHaveBeenCalledWith(expect.objectContaining({ resourceId: 'resource-1' })))
    fetchAbsences.mockClear()

    await user.click(screen.getByText('Camille DURAND'))

    await waitFor(() => expect(fetchAbsences).toHaveBeenCalledWith(expect.objectContaining({ resourceId: 'resource-2' })))
  })
})
