...
vi.mock('../../../api/endpoints/absences', () => ({ fetchAbsences, submitAbsence, validateAbsence, refuseAbsence, cancelAbsence }))
...
function absence(overrides: Partial<{ id: string; statut: number; dateDebut: string; dateFin: string; type: number }> = {}) {
  return {
    id: 'ab-1', resourceId: 'resource-1', type: 0, dateDebut: '2026-08-01', dateFin: '2026-08-03',
    demiJournee: false, commentaire: null, statut: 0, valideParIdentifiant: null, dateDecision: null,
    createdAt: '2026-07-20T00:00:00Z', createdBy: 's636140', ...overrides,
  }
}
...
describe('AbsencesPage', () => {
  it('displays the monthly/annual KPI from the availability endpoint, never summed client-side', async () => {
    ...
    fetchAvailability.mockResolvedValue({
      resourceId: 'resource-1', startDate: '2026-07-01', endDate: '2026-07-31', joursOuvres: 23, joursFeries: 0,
      joursAbsenceValidee: 3, capaciteTheorique: 178.25, capaciteReelle: 155, tauxDisponibilite: 87,
      chargeRunHeures: 0, chargeHorsRunHeures: 0,
    })
    ...
    expect(fetchAvailability).toHaveBeenCalledWith('resource-1', expect.any(String), expect.any(String))
  })
  ...
  it('re-queries when the statut filter changes', async () => {
    ...
    fetchAvailability.mockResolvedValue({
      resourceId: 'resource-1', startDate: '2026-07-01', endDate: '2026-07-31', joursOuvres: 23, joursFeries: 0,
      joursAbsenceValidee: 0, capaciteTheorique: 178.25, capaciteReelle: 178.25, tauxDisponibilite: 100,
      chargeRunHeures: 0, chargeHorsRunHeures: 0,
    })
    ...
    await waitFor(() => expect(fetchAbsences).toHaveBeenCalledWith(expect.objectContaining({ statut: 2 })))
  })
  ...
  it('shows Valider/Refuser on a Soumis absence, and refuses with an optional motif', async () => {
    ...
    await waitFor(() => expect(refuseAbsence).toHaveBeenCalledWith('ab-1', { commentaire: 'Charge insuffisante' }))
  })
  ...
  it('cancels a validated absence after confirmation', async () => {
    ...
    fetchAvailability.mockResolvedValue({
      resourceId: 'resource-1', startDate: '2026-07-01', endDate: '2026-07-31', joursOuvres: 23, joursFeries: 0,
      joursAbsenceValidee: 1, capaciteTheorique: 178.25, capaciteReelle: 170.5, tauxDisponibilite: 96,
      chargeRunHeures: 0, chargeHorsRunHeures: 0,
    })
