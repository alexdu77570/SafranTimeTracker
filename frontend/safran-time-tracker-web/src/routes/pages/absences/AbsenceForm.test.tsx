...
const { createAbsence, updateAbsence } = vi.hoisted(() => ({
  createAbsence: vi.fn(),
  updateAbsence: vi.fn(),
}))
...
describe('AbsenceCreateForm', () => {
  it('creates a Brouillon absence for the given resource', async () => {
    ...
    renderWithProviders(
      <AbsenceCreateForm resourceId="resource-1" onSuccess={onSuccess} onCancel={vi.fn()} />,
    )
...
describe('AbsenceEditForm', () => {
  ...
  it('sends the numeric type back to the API despite FormSelect using string values', async () => {
    ...
    await waitFor(() =>
      expect(updateAbsence).toHaveBeenCalledWith(
        '1',
        expect.objectContaining({ type: AbsenceType.Rtt }),
      ),
    )
  })
})
